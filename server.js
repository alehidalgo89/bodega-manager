...
<body>
    ...
    <script>
        ...
        async function init() {
            ...
            // Inicializar país en entrada
            document.getElementById('entPais').innerHTML = '<option value="">-Seleccionar País-</option>' + paises.map(p => '<option value="' + p.id + '">' + p.nombre + '</option>').join('');
            
            // Seleccionar el primer país automáticamente si existe
            if (paises.length > 0) {
                document.getElementById('entPais').value = paises[0].id;
                await cargarReg(); // Cargar regiones del primer país
            }
        ...
        async function cargarReg() {
            ...
            document.getElementById('entRegion').innerHTML = '<option value="">-Seleccionar Región-</option>' + regs.map(r => '<option value="' + r.id + '">' + r.nombre + '</option>').join('');
            
            // Auto-seleccionar la primera región
            if (regs.length > 0) {
                document.getElementById('entRegion').value = regs[0].id;
                await cargarTipo(); // Cargar tipos de la región
            }
        }
        ...
        async function cargarTipo() {
            ...
            // Auto-seleccionar el primer tipo válido
            if (tiposValidos.length > 0) {
                document.getElementById('entTipo').value = tiposValidos[0].id;
            }
