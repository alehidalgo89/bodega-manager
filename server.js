...
<body>
    ...
    <div class="container">
        ...
        <div id="movimientos" class="tab-content active">
            ...
            <div id="formMov" style="display:none;" class="section">
                <h3 id="movTit"></h3>
                <div style="background:rgba(255,165,0,0.2); padding:10px; margin:10px 0; border-radius:3px; border-left:4px solid #FFA500;">
                    <small>Busca por: Nombre, Bodega, Año O Código QR</small>
                </div>
                <input id="qrBusca" type="text" placeholder="Nombre/Bodega/Año/QR">
    ...
    <script>
        ...
        async function buscarPorQR() {
            ...
            const cantDiv = document.getElementById('cantidadDiv');
            
            // Primero intentar buscar por QR
            ...
            if (res.error) {
                ...
                const encontrados = allVinos.filter(v => 
                ...
                if (encontrados.length === 0) {
                    ...
                    // Si encuentra múltiples, cuenta cuántas disponibles (ubicacion_id != null)
                    const disponibles = encontrados.filter(v => v.ubicacion_id !== null).length;
                    res = encontrados[0]; // Usa el primer resultado como referencia
                    res.disponibles = disponibles;
                }
            } else {
                res.disponibles = 1; // Si encontró por QR, solo hay 1
            }
            
            if (res.error) {
                vi.innerHTML = '❌ NO ENCONTRADO - Intenta con nombre, bodega, año o QR';
                cantDiv.style.display = 'none';
            } else {
                vinoActual = res;
                const ub = res.zona ? res.zona + '(' + res.columna + ',' + res.fila + ')' : 'N/A';
                const maxCant = res.disponibles || 1;
                vi.innerHTML = res.nombre_vino + ' - ' + res.bodega + ' (' + res.ano + ') | ' + ub + '<br><strong>Disponibles: ' + maxCant + '</strong>';
                
                // Mostrar y limitar cantidad
                document.getElementById('cantidadMov').value = 1;
                document.getElementById('cantidadMov').max = maxCant;
                cantDiv.style.display = 'block';
            }
        ...
        async function registrarMovimiento() {
            ...
            const t = document.getElementById('movTipo').value;
            const cantidad = parseInt(document.getElementById('cantidadMov').value) || 1;
            ...
            try {
                // Si es por QR, solo registra 1 movimiento
                if (vinoActual.codigo_qr && cantidad === 1) {
                    ...
                    // Si es por búsqueda de nombre/bodega, registra "cantidad" movimientos
                    const allVinos = await fetch('/api/buscar').then(r => r.json());
                    const vinosDelMismo = allVinos.filter(v => 
                        v.nombre_vino === vinoActual.nombre_vino && 
                        v.bodega === vinoActual.bodega && 
                        v.ano === vinoActual.ano &&
                        v.ubicacion_id !== null
                    );
                    
                    const aRegistrar = vinosDelMismo.slice(0, cantidad);
                    
                    if (aRegistrar.length < cantidad) {
                        alert('Solo hay ' + aRegistrar.length + ' botella(s) disponible(s)');
                        return;
                    }
                    
                    for (const vino of aRegistrar) {
                        await fetch('/api/movimiento/' + vino.id, {
                            method: 'POST',
                            headers: {'Content-Type': 'application/json'},
                            body: JSON.stringify({ tipo: t })
                        });
                ...
                document.getElementById('movMsg').innerHTML = '<div class="msg ok">✅ ' + cantidad + ' movimiento(s) registrado(s)</div>';
                document.getElementById('qrBusca').value = '';
                document.getElementById('vinoInfo').style.display = 'none';
                document.getElementById('cantidadDiv').style.display = 'none';
        ...
        async function cargarInventario() {
            ...
            res.forEach(r => {
                ...
                const colorEstado = estado === 'Disponible' ? '#4CAF50' : (estado === 'Consumido' ? '#999' : '#f44336');
                html += '<tr><td>' + r.nombre_vino + '</td><td>' + r.bodega + '</td><td>' + r.ano + '</td><td>' + (r.tipo_nombre || '-') + '</td><td>' + ub + '</td><td style="color:' + colorEstado + '; font-weight:bold;">' + estado + '</td></tr>';
            });
            html += '</table>';
