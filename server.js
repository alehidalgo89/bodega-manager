...
app.post('/api/vinos', async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < cantReq; i++) {
            ...
            const ub = await client.query('SELECT u.id, u.zona_id, u.columna, u.fila FROM ubicaciones u WHERE u.disponible = true LIMIT 1 FOR UPDATE');
            ...
            const zona = await client.query('SELECT nombre FROM zonas WHERE id = $1', [ub1.zona_id]);
            const zona_nombre = zona.rows.length > 0 ? zona.rows[0].nombre : 'Z' + ub1.zona_id;
            ...
            vinos.push({
                ...
                ubicacion: {
                    ...
                    columna: ub1.columna,
                    fila: ub1.fila,
                    referencia: zona_nombre + '-' + ub1.columna + '-' + ub1.fila
                }
            });
        }
