...
app.post('/api/vinos', async (req, res) => {
    ...
    try {
        ...
        const disponibles = await client.query(
            'SELECT COUNT(*) as total FROM ubicaciones WHERE disponible = true'
        );
        
        const cantidadDisponible = parseInt(disponibles.rows[0].total);
        const cantidadRequerida = parseInt(cantidad);
        
        if (cantidadDisponible < cantidadRequerida) {
            await client.query('ROLLBACK');
            return res.json({ 
                error: `No hay suficientes ubicaciones disponibles. Disponibles: ${cantidadDisponible}, Requeridas: ${cantidadRequerida}` 
            });
        }
        ...
        for (let i = 0; i < cantidadRequerida; i++) {
            ...
            // Obtener ubicación disponible (DENTRO DE LA TRANSACCIÓN)
            ...
            if (ub.rows.length === 0) {
                await client.query('ROLLBACK');
                return res.json({ error: 'Error: Se agotaron las ubicaciones disponibles durante el proceso' });
            }
            ...
            await client.query('INSERT INTO movimientos (vino_id, tipo_movimiento, razon) VALUES ($1, $2, $3)',
                [result.rows[0].id, 'Entrada', 'Registro inicial']);
            
            vinosCreados.push({
                id: result.rows[0].id,
                codigo_qr: result.rows[0].codigo_qr,
                ubicacion: {
                    zona: ubicacion.zona_nombre || 'Zona ' + ubicacion.zona_id,
                    columna: ubicacion.columna,
                    fila: ubicacion.fila,
                    referencia: (ubicacion.zona_nombre || 'Z' + ubicacion.zona_id) + '-' + ubicacion.columna + '-' + ubicacion.fila
                }
            });
        }
        ...
        res.json({ 
            success: true, 
            message: `${cantidadRequerida} botella(s) registrada(s) exitosamente`,
            vinos: vinosCreados,
            disponiblesRestantes: cantidadDisponible - cantidadRequerida
        });
    } catch (err) {
        await client.query('ROLLBACK');
        res.json({ error: err.message });
...
app.get('/api/disponibilidad', async (req, res) => {
    try {
        const disponibles = await pool.query('SELECT COUNT(*) as total FROM ubicaciones WHERE disponible = true');
        const ocupadas = await pool.query('SELECT COUNT(*) as total FROM ubicaciones WHERE disponible = false');
        const total = await pool.query('SELECT COUNT(*) as total FROM ubicaciones');
        
        res.json({
            disponibles: parseInt(disponibles.rows[0].total),
            ocupadas: parseInt(ocupadas.rows[0].total),
            total: parseInt(total.rows[0].total),
            porcentajeOcupado: ((parseInt(ocupadas.rows[0].total) / parseInt(total.rows[0].total)) * 100).toFixed(1)
        });
    } catch (err) {
        res.json({ error: err.message });
    }
...
<body>
    ...
    <div class="container">
        ...
        <div id="movimientos" class="tab-content active">
            ...
            <div id="formEntrada" style="display:none;">
                ...
                <div style="background: rgba(212,175,55,0.15); padding: 10px; margin: 15px 0; border-left: 4px solid #D4AF37; border-radius: 3px;">
                    <strong id="disponibilidadInfo">Cargando disponibilidad...</strong>
                </div>
                ...
                <div id="advertenciaCapacidad" style="display:none; background: #ff9800; padding: 10px; margin: 10px 0; border-radius: 3px;"></div>
                <label>Notas</label>
    ...
    <script>
        ...
        async function cargarDisponibilidad() {
            try {
                const data = await fetch('/api/disponibilidad').then(r => r.json());
                const info = document.getElementById('disponibilidadInfo');
                if (data.disponibles !== undefined) {
                    info.innerHTML = `<strong style="color: #4CAF50;">✅ Disponibles:</strong> ${data.disponibles} espacios | <strong style="color: #ff9800;">📦 Ocupados:</strong> ${data.ocupadas} | <strong>Total Capacidad:</strong> ${data.total} (${data.porcentajeOcupado}% ocupado)`;
                } else {
                    info.innerHTML = 'No se pudo cargar disponibilidad';
                }
            } catch (err) {
                document.getElementById('disponibilidadInfo').innerHTML = 'Error cargando disponibilidad';
            }
        }
        ...
        function validarCantidad() {
            ...
            fetch('/api/disponibilidad').then(r => r.json()).then(data => {
                if (cantidad > data.disponibles) {
                    advertencia.innerHTML = `⚠️ <strong>Advertencia:</strong> Solo hay ${data.disponibles} espacios disponibles, pero intentas guardar ${cantidad} botellas`;
        ...
        async function registrarEntrada() {
            ...
            if (res.success) {
                let html = '<strong style="color: #4CAF50;">✅ ' + res.message + '</strong><br><br>';
                html += '<strong style="color: #D4AF37;">Espacios restantes: ' + res.disponiblesRestantes + '</strong><br><br>';
                res.vinos.forEach((v, i) => {
                    html += '<div style="background: rgba(76,175,80,0.15); padding: 15px; margin: 10px 0; border-left: 4px solid #4CAF50; border-radius: 3px;">';
